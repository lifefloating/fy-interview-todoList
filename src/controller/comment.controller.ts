...
import commentService from '../service/remark.service';
...
async function create(req: Request, res: Response) {
  const { replyUser, todoId, content } = req.body;
  if (!todoId) {
    return res.status(401).send([]);
  }

  try {
    const comment = await commentService.create({ replyUser, todoId, content });
